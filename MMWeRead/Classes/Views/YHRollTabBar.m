...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _selectLine.backgroundColor = color_naviTitle;
...
//    __block CGFloat allLength = self.sectionLeft + self.sectionRight - self.itemSpacing;
    __block CGFloat allLength = 0;
    __block CGFloat length = 0.0;
    for (int i = 0; i < rows; i++) {
        NSString *title = [_delegate rollTabBarView:self titleForRow:i];
        CGRect rect = [title boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 20) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.0]} context:nil];
        allLength += rect.size.width + self.itemSpacing;
        length += rect.size.width;
    }
    if (allLength < self.width && rows > 1) {
//        CGFloat aSpacing = (self.width - self.sectionLeft - self.sectionRight - length) / (rows - 1);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGRect rect = [[_delegate rollTabBarView:self titleForRow:indexPath.row] boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 16) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil];
...
- (void)updateCell:(UICollectionViewCell *)cell isSelected:(BOOL)isSelected {
    UILabel *textLabel = (UILabel*)[cell.contentView viewWithTag:5];
    if (isSelected) {
        textLabel.textColor = color_naviTitle;
    } else {
        textLabel.textColor = color_grayline;
    }
}
